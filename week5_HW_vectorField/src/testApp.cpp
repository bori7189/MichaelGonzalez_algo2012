...
void testApp::update(){
	...
	for (int i = 0; i < particles.size(); i++){
		...
		for (int j = 0; j < i; j++){
			particles[i].addRepulsionForce(particles[j], 75, 0.4);
	...
	for (int i = 0; i < particles.size(); i++){
        ...
        particles[i].checkLoc();
        //cout << particles[i].pos.x << " and " << particles[i].pos.y << endl;
	}
	
    
	
	//if (bFade == true) VF.fadeField(0.001f);
    
    int width = ofRandom(0,ofGetWidth());
    int height = ofRandom(0, ofGetHeight());
   // VF.addOutwardCircle(width, height, 100, 0.5f);
    
    count++;
    if(count != 0 && count%30 == 0){
       //VF.randomizeField(0.75f);
        count = 0;
    }
    //VF.addInwardCircle(width, height , 90, 0.3f);
